...
import { api, Customer, IncomeCategory, Transaction, INCOME_CATEGORIES } from "../../lib/api";

type Step = "amount" | "category" | "customer" | "homeuse";
type PendingMethod = "CASH" | "TRANSFER" | "DEBT" | null;
...
export default function RapidEntryPage() {
  ...
  const [pendingCategory, setPendingCategory] = useState<IncomeCategory | null>(null);
  ...
  // Step 1 -> 2: choose cash/transfer/debt, then always pick a category next
  function chooseMethod(method: "CASH" | "TRANSFER" | "DEBT") {
    if (!numericAmount) return;
    setPendingMethod(method);
    setStep("category");
  }

  // Step 2 -> save (cash/transfer) or -> step 3 (debt needs a customer too)
  async function chooseCategory(category: IncomeCategory) {
    if (pendingMethod === "DEBT") {
      setPendingCategory(category);
      setStep("customer");
      return;
    }
    if (pendingMethod === "CASH" || pendingMethod === "TRANSFER") {
      await saveIncome(pendingMethod, category);
    }
  }

  async function saveIncome(method: "CASH" | "TRANSFER", category: IncomeCategory, customer?: Customer) {
    setSaving(true);
    try {
      await api.createIncome([
        { amount: numericAmount, payment_method: method, income_category: category },
      ]);
      const methodLabel = method === "CASH" ? "เงินสด" : "เงินโอน";
      const categoryLabel = INCOME_CATEGORIES.find((c) => c.value === category)?.label;
      showToast(`บันทึก ${numericAmount} บาท (${methodLabel} · ${categoryLabel})`);
      clearAll();
      refreshToday();
    } finally {
      setSaving(false);
  ...
  async function saveDebt(customer: Customer) {
    if (!numericAmount || !pendingCategory) return;
    setSaving(true);
    try {
      await api.createIncome([
        {
          ...
          income_category: pendingCategory,
        },
      ]);
      const categoryLabel = INCOME_CATEGORIES.find((c) => c.value === pendingCategory)?.label;
      showToast(
        `บันทึก ${numericAmount} บาท (แปะโป้ง: ${customer.nickname || customer.name} · ${categoryLabel})`
      );
      clearAll();
      refreshToday();
    } finally {
  ...
  const labelFor = (t: Transaction) => {
    if (t.entry_type === "INCOME") {
      ...
      if (t.payment_method === "CASH") return `เงินสด${suffix}`;
      if (t.payment_method === "TRANSFER") return `เงินโอน${suffix}`;
  ...
  return (
    <div className="flex flex-col gap-6">
      <header>
        ...
        <p className="text-ink/60">พิมพ์ยอดเงิน แล้วเลือกช่องทาง แล้วเลือกหมวด</p>
      </header>
      ...
      {step === "category" && (
        <div className="flex flex-col gap-3">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-semibold">ของหมวดไหนคะ</h2>
          ...
          <div className="grid grid-cols-1 gap-3">
            {INCOME_CATEGORIES.map((cat) => (
              <button
                ...
                disabled={saving}
                className={`h-20 rounded-xl ${cat.color} text-xl font-bold text-white active:opacity-90`}
              >
                {cat.label}
              </button>
            ))}
      ...
      {step === "customer" && (
        <div className="flex flex-col gap-3">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => setStep("category")} className="text-ink/60 underline">
              ย้อนกลับ
            </button>
          </div>
          <div className="grid grid-cols-2 gap-3">
